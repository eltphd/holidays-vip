...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const kit = await getKit("whole-table", slug).catch(() => null);
  return kit ? { title: `${kit.display_name} · Whole Table`, description: kit.summary ?? undefined } : {};
}

export default async function WholeTableKitPage({ params }: Params) {
  if (!catalogV2Enabled()) notFound();
  const { kit: slug } = await params;
  const [category, kit] = await Promise.all([getCategory("whole-table"), getKit("whole-table", slug)]);
